...
import org.araqne.auth.api.AuthCallback;
import org.araqne.auth.api.AuthProfile;
import org.araqne.auth.api.AuthProvider;
import org.araqne.auth.api.AuthService;
import org.araqne.auth.api.AuthStrategy;
import org.araqne.auth.api.EmptyAuthCallback;
import org.araqne.auth.api.UserCredentials;
import org.araqne.auth.api.UserPrincipal;
...
public class DefaultAuthService implements AuthService {
	...
	public void createProfile(AuthProfile p) {
		...
		ConfigDatabase db = conf.getDatabase("araqne-core");
		ConfigCollection col = db.ensureCollection("auth-profile");
		col.add(PrimitiveConverter.serialize(p), "araqne-core", "created auth profile: " + p.getName());
	}
	...
	public void updateProfile(AuthProfile p) {
		...
		if (c != null) {
			...
			col.update(c, false, "araqne-core", "created auth profile: " + p.getName());
		}
	}
	...
	public void removeProfile(String name) {
		...
		if (c != null)
			col.remove(c, false, "araqne-core", "removed auth profile: " + name);
	}
