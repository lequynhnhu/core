...
Name "Araqne Core"
Caption "Araqne Core"
OutFile "Araqne-Core-1.7.0-Setup.exe"
...
;    MessageBox MB_OK "$R1"
	...
	${Endif}
;	MessageBox MB_OK "$R0"
	ExecWait `"$R0$\" //IS//$\"Araqne_Core$\" --Install=$\"$R0$\" --Description=$\"Araqne Core 1.7.0$\" --Jvm=$\"$R1$\" --Startup=auto --StartMode=jvm --Classpath=$\"$INSTDIR\core\araqne-core-1.7.0-package.jar$\" --StartClass=org.araqne.main.Araqne --StartMethod=windowsService --StartParams=start --StopMode=jvm --StopClass=org.araqne.main.Araqne --StopMethod=windowsService --StopParams=stop --LogPath=$\"$INSTDIR\log$\" --StdError=auto --StdOutput=auto --StartPath=$\"$INSTDIR"`
